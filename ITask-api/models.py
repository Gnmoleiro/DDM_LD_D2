...
class Gere(db.Model):
    __tablename__ = 'gere'
    idProgramador = db.Column(db.Integer, db.ForeignKey('programador.idUser'), primary_key=True)
    idGestor = db.Column(db.Integer, db.ForeignKey('gestor.idUser'), primary_key=True)

class TipoTarefa(db.Model):
    __tablename__ = 'tipoTarefa'
    idTipoTarefa = db.Column(db.Integer, primary_key=True, nullable=False)
    nome = db.Column(db.String(100), nullable=False)

class Tarefa(db.Model):
    __tablename__ = 'tarefa'
    idTarefa = db.Column(db.Integer, primary_key=True, autoincrement=True)
    idGestor = db.Column(db.Integer, db.ForeignKey('gestor.idUser'), nullable=False)
    idProgramador = db.Column(db.Integer, db.ForeignKey('programador.idUser'), nullable=False)
    ...
    idTipoTarefa = db.Column(db.Integer, db.ForeignKey('tipoTarefa.idTipoTarefa'), nullable=False)
