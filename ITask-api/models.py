...
class Gestor(db.Model):
    ...
    idDono = db.Column(db.Integer, db.ForeignKey('dono.idUser'), nullable=False)
...
class Programador(db.Model):
    ...
    idUser = db.Column(db.Integer, db.ForeignKey('users.idUser'), primary_key=True)
    idGestor = db.Column(db.Integer, db.ForeignKey('gestores.idUser'), nullable=False)
...
class TipoTarefa(db.Model):
    ...
    idTipoTarefa = db.Column(db.Integer, primary_key=True, autoincrement=True)
    nome = db.Column(db.String(100), nullable=False)
...
class Tarefa(db.Model):
    ...
    idGestor = db.Column(db.Integer, db.ForeignKey('gestores.idUser'), nullable=False)
    idProgramador = db.Column(db.Integer, db.ForeignKey('programadores.idUser'), nullable=False)
    idTipoTarefa = db.Column(db.Integer, db.ForeignKey('tipos_tarefa.idTipoTarefa'), nullable=False)
